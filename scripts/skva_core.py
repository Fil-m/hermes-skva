...
Output YAML with skill content."""
    ...
    # SelfImprover: extract concrete improvements + core patches
    improver = SelfImprover(project_dir)
    await improver.generate(response, articles)
    # Try to patch the core if improvement found
    patched = await improver.auto_improve_core(response, articles)
    if patched:
        log(f"  ⚡ Core auto-patched! Restart SKVA to apply.")
...
```"""
    ...
    SKVA_CORE = os.path.abspath(__file__)

    async def generate_core_patch(self, research_output: str) -> dict:
        """Generate a concrete Python patch for skva_core.py from research."""
        # Read current core file for context
        with open(self.SKVA_CORE) as f:
            core_code = f.read()

        prompt = f"""You are patching SKVA's own source code based on tech research.

TASK: Generate ONE small, precise improvement for skva_core.py.
Focus on: error handling, performance, security, or a new feature.
Keep the change SMALL (<30 lines changed total).

Rules:
1. Use SEARCH/REPLACE format (old exact code → new code)
2. The change MUST be safe (no syntax errors, no breaking changes)
3. Only modify ONE location
4. Add a comment "# [SKVA-AUTO] description" near the change

CORE FILE (first 500 lines):
{core_code[:2000]}
...
```python
# [SKVA-AUTO] what this patch does
<<<<<<< SEARCH
old code
=======
new code
>>>>>>> REPLACE
```"""
        response, in_tok, out_tok = await gonka_call(prompt, timeout=120)
        if not response:
        ...
        if not sr:
            return {"status": "no_patch", "response": response[:500]}

        old, new = sr.group(1), sr.group(2)
        desc_match = re.search(r"# \[SKVA-AUTO\] (.+)", response)
        desc = desc_match.group(1) if desc_match else "auto-patch"

        # Save patch file
        patch_file = self.improvements_dir / f"core_patch_{int(time.time())}.patch"
        patch_file.write_text(f"# {desc}\n<<<<<<< SEARCH\n{old}\n=======\n{new}\n>>>>>>> REPLACE\n")
        log(f"  Core patch generated: {desc[:80]}")

        return {"status": "generated", "desc": desc, "old": old, "new": new, "file": str(patch_file)}

    def validate_patch(self, old: str, new: str) -> bool:
        """Validate that a patch won't break the codebase."""
        # Try to apply the patch in memory and check syntax
        with open(self.SKVA_CORE) as f:
            content = f.read()
        if old not in content:
            log(f"  Patch validation FAILED: old code not found in file", "WARN")
            return False
        patched = content.replace(old, new, 1)
        try:
            compile(patched, 'skva_core.py', 'exec')
            log(f"  Patch validation: syntax OK")
            return True
        except SyntaxError as e:
            log(f"  Patch validation FAILED: {e}", "WARN")
            return False

    def apply_patch(self, old: str, new: str, desc: str = "auto") -> bool:
        """Apply a validated patch to skva_core.py on disk."""
        with open(self.SKVA_CORE) as f:
            content = f.read()
        if old not in content:
            log(f"  Apply FAILED: old code not found", "WARN")
            return False
        new_content = content.replace(old, new, 1)
        # Backup
        backup = self.SKVA_CORE + ".bak"
        import shutil
        shutil.copy2(self.SKVA_CORE, backup)
        # Write
        with open(self.SKVA_CORE, 'w') as f:
            f.write(new_content)
        log(f"  ✅ Core patched: {desc[:80]} (backup at {backup})")
        return True

    async def auto_improve_core(self, research_output: str, trend_articles: list) -> bool:
        """Full pipeline: research → generate patch → validate → apply."""
        result = await self.generate_core_patch(research_output)
        if result.get("status") != "generated":
            return False
        if not self.validate_patch(result["old"], result["new"]):
            return False
        return self.apply_patch(result["old"], result["new"], result.get("desc", "auto"))


# ═══════════════════════════════════════════════════
# DOCUMENT CHUNKER — розбиття великих ТЗ/документів
# ═══════════════════════════════════════════════════

class DocumentChunker:
    """Splits large documents into chunks, summarizes each, merges context."""

    def __init__(self, max_chunk_size: int = 12000, overlap: int = 1000):
        self.max_chunk_size = max_chunk_size
        self.overlap = overlap

    def split(self, text: str) -> list:
        """Split text at section boundaries (## headings). Each chunk is complete sections."""
        lines = text.split("\n")
        chunks = []
        current = []
        current_size = 0
        ...
        for i, line in enumerate(lines):
            ...
            # If this is a new section and current chunk would overflow
            if is_section and current_size + line_size > self.max_chunk_size and current:
                chunks.append("\n".join(current))
                # Overlap: keep last 2 sections
                overlap_lines = []
                for l in reversed(current):
                    overlap_lines.insert(0, l)
                    if l.startswith("## ") and len(overlap_lines) > 3:
                        break
    ...
    async def summarize_chunk(self, chunk: str, index: int, total: int) -> str:
        """Send one chunk to Gonka for summarization."""
        prompt = f"""Summarize this section of a Technical Specification (part {index+1}/{total}).

Extract: key requirements, data structures, UI elements, constraints.
Focus on what a developer needs to implement this section.

```text
{chunk[:10000]}
```

Output: concise summary in Ukrainian, max 500 chars. Focus on actionable requirements."""
        response, in_tok, out_tok = await gonka_call(prompt, timeout=60)
        summary = response[:1000] if response else f"[Part {index+1}/{total} — no summary]"
        log(f"  Chunk {index+1}/{total}: {in_tok}→{out_tok} tok")
        return f"=== ЧАСТИНА {index+1}/{total} ===\n{summary}\n"

    async def process(self, text: str) -> str:
        """Full pipeline: split → summarize chunks in parallel → merge."""
        chunks = self.split(text)
        log(f"📄 Document: {len(text)} chars → {len(chunks)} chunks")

        if len(chunks) <= 1:
            return text  # Small enough, use as-is

        # Summarize all chunks in parallel (max 3 concurrent)
        summaries = []
        for i in range(0, len(chunks), 3):
            batch = chunks[i:i+3]
            results = await asyncio.gather(*[
                self.summarize_chunk(c, i + j, len(chunks))
        ...
        # Merge all summaries
        merged = "\n\n".join(summaries)
        log(f"  Merged summary: {len(merged)} chars from {len(chunks)} chunks")
        return merged
...
class MarkdownAgent:
    ...
    def __init__(self, role, system_prompt, task_prompt, project_dir, 
        ...
        # Document Chunker: auto-split large prompts
        if len(task_prompt) > 15000:
            log(f"  Large prompt ({len(task_prompt)} chars), chunking...")
            chunker = DocumentChunker()
            processed = asyncio.get_event_loop().run_until_complete(chunker.process(task_prompt))
            log(f"  Chunked: {len(task_prompt)} → {len(processed)} chars")
            task_prompt = processed
